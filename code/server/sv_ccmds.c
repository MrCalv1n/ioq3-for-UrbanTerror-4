...
static char *SV_GetMapSoundingLike(const char *s)
{
    ...
    if (count == 1)
        return matches[0]; // @p5yc0runn3r: matches points to static string, safe to return.
...
static void SV_StopRecordAll(void)
{
    ...
    for (slot = 0, client = svs.clients; slot < sv_maxclients->integer; slot++, client++)
    {
        ...
        if (client->netchan.remoteAddress.type == NA_BOT || client->state != CS_ACTIVE // disconnects are handled elsewhere
...
void SV_CamperMode_fc(void)
{
    ...
    if (Cmd_Argc() != 4)
    {
        Com_Printf("Usage: campermode <On/Off> <loop> <count>\n");
        Com_Printf("0 = Off; 1 = On\n");
        return;
    }
...
void SV_AddOperatorCommands(void)
{
    ...
    Cmd_AddCommand("campermode", SV_CamperMode_fc);
